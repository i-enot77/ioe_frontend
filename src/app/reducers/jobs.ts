...
export const addJob = createAsyncThunk(
  ...
  async (jobData: JobProp) => {
    const response = await axios.post("http://localhost:3500/jobs", jobData);
    return response.data;
  }
);
...
export const updatePeriodicJob = createAsyncThunk(
  ...
  async (periodicJobData: JobProp) => {
    ...
    const res = await axios.put(`http://localhost:3500/jobs/${id}`, data);
    return { ...res.data };
  }
);
...
export const jobSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      .addCase(
        fetchJobs.fulfilled,
        (state, action: PayloadAction<JobProp[]>) => {
          state.jobsArr = action.payload;
        }
      )
      .addCase(
        fetchJobItem.fulfilled,
        (state, action: PayloadAction<JobProp>) => {
          state.periodicJobData = action.payload;
        }
      )
      .addCase(
        addPeriodicJob.fulfilled,
        (state, action: PayloadAction<JobProp>) => {
      ...
      .addCase(
        updatePeriodicJob.fulfilled,
        (state, action: PayloadAction<JobProp>) => {
          state.jobsArr = state.jobsArr.map((item) =>
            item.id === state.periodicJobData.id ? action.payload : item
          );
          state.periodicJobData = initialState.periodicJobData;
        }
      )
      .addCase(deleteJob.fulfilled, (state, action) => {
        state.jobsArr = state.jobsArr.filter(
